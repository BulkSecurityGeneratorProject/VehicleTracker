...
describe('Order e2e test', function () {
    ...
    it('should load Orders', function () {
        entityMenu.click();
        element(by.css('[ui-sref="order"]')).click().then(function() {
            expect(element.all(by.css('h2')).first().getText()).toMatch(/Orders/);
        });
    });

    it('should load create Order dialog', function () {
        element(by.css('[ui-sref="order.new"]')).click().then(function() {
            expect(element(by.css('h4.modal-title')).getText()).toMatch(/Create or edit a Order/);
